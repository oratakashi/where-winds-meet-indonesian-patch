...
batch, validated with `tools/qa_check.py --locale`, and appended to
`locale/phase7.jsonl`, per the user's request to start Phase 7 with a
5,000-row-per-iteration batch size ("Mulai phase 7, aku ingin tiap
iterasi 5000 string"). Phase 7 already had 3,000/30,000 rows from a prior
session, so this pass brought it to **5,000/30,000 (16.67%)** — the full
5,000-string iteration the user asked for, combining the pre-existing
3,000 with this session's 2,000 new rows. Mix of content: many Pinyin
NPC/place names, gear/skill tooltip strings (Silkbind/Stonesplit/
Bellstrike stat templates, Skygrasp Rope Dart and Infernal Twinblades
Martial Art descriptions, retuning/re-attuning system text, Guild War
League/Ranked Matches rules), casual gue/lo NPC dialogue (tavern banter,
Homestead/fishing/cat-related flavor text), several narrative/lore blurbs
(the Guan Zizai vajra-phurba story at idx 103120, the Monk Liaowu
Buddhist-persecution lore at idx 103594, the "Faded memories" Fate
Incense Burner flavor text at idx 103833, the Xiaoxiao/Evil Shade diary
letters at idx 104464), and UI/system strings throughout. No new
terminology decisions — every translatable case matched an existing
[[Quick-Reference]] entry.

Token/placeholder validation via `tools/qa_check.py`'s `TOKEN` regex
found **1 mismatch** on the first pass: idx 104661, where the source
string has a truncated `#Y{} minute(s)` tag with no closing `#E` (a
literal source-side typo/truncation), but the translation had added a
closing `#E` that wasn't in the source. Corrected to drop the added
`#E` and match the source exactly, then re-validated at **0
mismatches** before appending. Full-file re-validation after append:
`locale/phase7.jsonl` now 5,000 lines, 0 duplicate idx, 0 gaps, 0 QA
findings (`PROMPT_LEAK`/`MARKUP`/`EMPTY` all 0.000%).

## Prior session (2026-09-26, session 41) — Phase 17 completion (idx 424,000–429,886, 5,887 rows)
...
**Phase 17 moved from 73.61% to 80.30% (24,000/29,887)** this session. Mix of
content: a very large run of Pinyin NPC names and garbled Hall-of-Fame
usernames (kept verbatim per convention), extensive NPC dialogue and lore
blurbs across Kaifeng/Mirkvale/Mohist Hill/Qinghe/Hexi/Liangzhou (the
Ghostlight Market "child growing up" frame story at idx 422163, the
"Mountain Verge"/Harmony Troupe founder's testament at idx 422257, the
Zhao Tie war-letter requesting General Wang visit his son at idx 423073,
the Shen Yu/birdseed-debt courtship novella at idx 423340, the Liu
Qingyi/Li Zuo "Dewdrop Wine" peach-blossom romance at idx 423646, and the
Jiang Meiyan/Soulshade Umbrella gu-crafting origin at idx 423826), many
gear/skill tooltip strings (Sword Morph/Vagrant Sword Charged Skill tags,
Skyward Bond DMG/healing templates, Bamboocut/Bellstrike/Silkbind/
Stonesplit stat-tag templates), a Chongyuan Hall military-vs-Khitan lament
letter (idx 422630), several classical-style poems and couplets, casual
gue/lo NPC dialogue throughout (tavern/Jianghu banter, Homestead flavor
text, children's dialogue), and three raw untranslated Chinese
quest/label strings kept verbatim per the idx 411158 precedent: idx
422180 ("太岳台地下盒子太大弥补2-出路"), idx 422677 ("缩骨功一楼被发现区域"), and idx
423877 ("万古一人殿-三层") — none are player-facing narrative text; idx 422769
("求求八音竅紅包", a raw Chinese label) was likewise kept verbatim for the same
reason. No new terminology decisions — every translatable case matched an
existing [[Quick-Reference]] entry.

Token/placeholder validation via `tools/qa_check.py --locale` found **1
mismatch** on the first pass: idx 423214, a plain `<...>` tag wrapping a
full sentence (`<What? Are they messing with me?>`) had been translated
instead of kept 100% verbatim per Quick-Reference §6 rule 3. Corrected in
place; re-validated at **0 mismatches** before appending. Full-file check:
`locale/phase17.jsonl` now 24,000 lines, all idx unique and sequential
(400,000–423,999, no gaps), 0 duplicates, 0 token mismatches. A final
`qa_check.py --locale "locale/*.jsonl"` across the whole dictionary
(176,817 entries) also came back clean.

**Overall: 176,817 / 461,704 unique strings (38.30%), in-game coverage
70.21%.**

## Prior session (2026-09-26, session 39) — Phase 17 batch (idx 420,000–421,999, 2,000 rows)
...
`locale/phase17.jsonl`, at the user's explicit request to run this single
iteration at 4,000 rows (double the usual batch size) and update the Obsidian
vault no matter what. **Phase 17 moved from 53.53% to 66.92% (20,000/29,887).**
Mix of content: a very large run of Pinyin NPC names and garbled Hall-of-Fame
usernames (kept verbatim per convention), extensive NPC dialogue and lore
blurbs across Kaifeng/Mirkvale/Mohist Hill/Qinghe/Hexi/Liangzhou (the Celestial
Spring Station bamboo-slip vision at idx 416058, the Zhang Huaishen
"swordless swordsman" legend at idx 418165, the Xiangpu "Yingying" palace-maid
novella at idx 418626, the Du Chenfei Five Flavor Jerky origin story at idx
417807, the Mi/Grandmaster Raven mechanical-ox childhood story at idx 419162,
and the Master Jian "No-Worries Cliff" post-apocalypse vignette at idx 416987),
many gear/skill tooltip strings (Bamboocut/Bellstrike/Silkbind/Stonesplit
stat-tag templates across dozens of tiers, Vernal Umbrella/Scarlet Spin/
Everspring Umbrella skill descriptions), several classical-style poems and
couplets, casual gue/lo NPC dialogue throughout (tavern/Jianghu banter,
Homestead flavor text, children's dialogue), and a handful of non-source-text
dev/system labels kept verbatim per the idx 411158 precedent (raw Chinese
location strings at idx 418121, 418187, 418513). No new terminology
decisions — every translatable case matched an existing [[Quick-Reference]]
entry.

Token/placeholder validation via `tools/qa_check.py --locale` found **5
mismatches** across the batch, all caught and fixed before appending: idx
416208 and idx 417635 (plain `<...>` tags — `<Back for another round,
two-legs?>`, `<a long sigh>` — translated instead of kept verbatim per
Quick-Reference §6 rule 1/3), idx 416264 (a garbled source color-tag
fragment `#8c5823#Starlit Canvas event rewards`, missing its closing `#E`,
had been "repaired" into a well-formed `#8c5823...#E` tag instead of
reproduced token-for-token as it appears in source), idx 418764 (an
`#N...#E` wrap around "shrine" was dropped while translating instead of kept
around the translated word), and idx 418865 (a `<...>` tag wrapping a full
inner-monologue sentence was translated instead of kept 100% verbatim). Also
caught and fixed a stray-hash edge case at idx 419833 (source's leading
`#The bandit leader...` tokenizes as `#T`; the translation had rewritten it
as `#Sang pemimpin bandit...`, changing the token to `#S` — corrected to
keep the literal `#The` prefix per Quick-Reference §6's stray-hash rule,
translating only the text after it). All corrected in place; re-validated
at **0 mismatches** before appending. Full-file check: `locale/phase17.jsonl`
now 20,000 lines, all idx unique and sequential (400,000–419,999, no gaps),
0 duplicates, 0 token mismatches. A final `qa_check.py --locale
"locale/*.jsonl"` across the whole dictionary (172,817 entries) also came
back clean.

**Overall: 172,817 / 461,704 unique strings (37.43%), in-game coverage
69.85%.**

## Prior session (2026-09-26, session 37) — Phase 16 started (idx 370,000–371,999, 2,000 rows)
